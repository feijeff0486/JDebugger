...
public final class CustomActivityOnCrash {
    ...
    public static final String CRASH_CATCHER_PACKAGE = "com.jeff.jdebugger.crashcatcher";
    /**
     * log只存在三天
     */
    private static final long LOG_EXIST_TIME = 259200;

    //Extras passed to the error activity
    private static final String EXTRA_CONFIG = CRASH_CATCHER_PACKAGE + ".EXTRA_CONFIG";
    private static final String EXTRA_STACK_TRACE = CRASH_CATCHER_PACKAGE + ".EXTRA_STACK_TRACE";
    private static final String EXTRA_ACTIVITY_LOG = CRASH_CATCHER_PACKAGE + ".EXTRA_ACTIVITY_LOG";

    //General constants
    private static final String INTENT_ACTION_ERROR_ACTIVITY = CRASH_CATCHER_PACKAGE + ".ERROR";
    private static final String INTENT_ACTION_RESTART_ACTIVITY = CRASH_CATCHER_PACKAGE + ".RESTART";
    private static final String CRASH_HANDLER_PACKAGE_NAME = CRASH_CATCHER_PACKAGE;
    ...
    private static CatcherConfig config = new CatcherConfig();
                        ...
                        public void uncaughtException(Thread thread, final Throwable throwable) {
                            if (config.isEnabled()) {
                                ...
                                if (hasCrashedInTheLastSeconds(application)) {
                                    ...
                                    } else if (config.getBackgroundMode() == CatcherConfig.BackgroundMode.BACKGROUND_MODE_SHOW_CUSTOM || !isInBackground) {
                                    ...
                                    } else if (config.getBackgroundMode() == CatcherConfig.BackgroundMode.BACKGROUND_MODE_CRASH) {
                                        if (oldHandler != null) {
                                            oldHandler.uncaughtException(thread, throwable);
                                            return;
                                        }
     ...
     */
    @Nullable
    public static CatcherConfig getConfigFromIntent(@NonNull Intent intent) {
        CatcherConfig config = (CatcherConfig) intent.getSerializableExtra(CustomActivityOnCrash.EXTRA_CONFIG);
     ...
     */
    public static void restartApplicationWithIntent(@NonNull Activity activity, @NonNull Intent intent, @NonNull CatcherConfig config) {
    ...
    public static void restartApplication(@NonNull Activity activity, @NonNull CatcherConfig config) {
     ...
     * @param config   The config object as obtained by calling getConfigFromIntent.
     */
    public static void closeApplication(@NonNull Activity activity, @NonNull CatcherConfig config) {
        if (config.getEventListener() != null) {
    ...
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    @NonNull
    public static CatcherConfig getConfig() {
        return config;
    }
     ...
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public static void setConfig(@NonNull CatcherConfig config) {
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="com.jeff.jdebugger.crashcatcher.RESTART" />,
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="com.jeff.jdebugger.crashcatcher.RESTART" />,
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="com.jeff.jdebugger.crashcatcher.ERROR" />,
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="com.jeff.jdebugger.crashcatcher.ERROR" />,
     * If there is no activity with that intent filter, this returns null.
     *
     * @param context A valid context. Must not be null.
     * @return A valid activity class, or null if no suitable one is found
    ...
    private boolean handleException(final Throwable ex) {
        ...
        Log.i(TAG, "handleException: crashed! ");

        File file = new File(application.getCacheDir().getAbsolutePath() + "/error/");
        if (!file.exists()) {
            file.mkdirs();
        } else {
            ...
            if (files != null && files.length > 0) {
                Log.i(TAG, String.format("handleException: there are %s error log.", files.length));
                ...
                for (int i = 0; i < fileList.size(); i++) {
                    ...
                    long writeTime = Long.valueOf(f.getName().replace(".log", ""));
                    if (i <= fileList.size() - 5 || current - writeTime >= LOG_EXIST_TIME) {
                        f.delete();
                    }
                }
            }
        ...
        //保存错误日志到本地：/data/data/<package>/cache/error/xxx.log
        try {
            String logName = application.getCacheDir().getAbsolutePath()
                    + "/error/" + System.currentTimeMillis() + ".log";
            PrintWriter writer = new PrintWriter(logName);
            ex.printStackTrace(writer);
            writer.close();
            Log.i(TAG, String.format("handleException: save error info to %s.", logName));
        } catch (Exception e) {
            e.printStackTrace();
        }

        // TODO: 2020/7/30 upload error info...
        return true;
    }
